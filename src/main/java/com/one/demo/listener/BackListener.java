...
public class BackListener {
    ...
    public void checkListenerHandel(CheckApplyEvent checkApplyEvent){
        if(!checkApplyEvent.getTaskEntity().backCheckStatus()){
            return;
        }
        ...
        if( checkApplyEvent.getActionType() == 1 ){ // 审核通过
            ...
            applyEntity.backFinish();
        }else if ( checkApplyEvent.getActionType() == 2 ){ //审核不通过
            checkApplyEvent.getTaskEntity().checkFault(checkApplyEvent.getUserlable(),"申请图书不能归还");
            applyEntity.backCancel();
        } else{
    ...
    public void cancelListenerHandel(CancelApplyEvent cancelApplyEvent) {
        if(!cancelApplyEvent.getTaskEntity().backCheckStatus()){
            return;
        }
